...
export default function SingUp({ onClose }: SingUpProps) {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        if (REGEX_NAME.test(name) === false) {
            setError("Name must First letter capital and contain only letters  ");
            return;
        }
        ...
        const tPassword = password.trim(); 0
        ...
        <div className=" md:w-96  mx-auto fixed inset-0 flex items-start justify-center mt-16  z-10">
            ...
            >
                ...
                <div className="relative">
                    <input type={showPassword ? "text" : "password"} className="border p-2 w-full rounded" onChange={(e) => setPassword(e.target.value)} value={password} />
                    <button
                        type="button"
                        onClick={Showpassword}
                        className="absolute right-2 top-2.5 text-sm text-gray-600"
                    >
                ...
                <div className="relative">
                    <input type={showConPassword ? "text" : "password"} className="border p-2 w-full rounded" onChange={(e) => setConfirmPassword(e.target.value)} value={confirmPassword} />
                    <button
                        type="button"
                        onClick={ShowConpassword}
                        className="absolute right-2 top-2.5 text-sm text-gray-600"
                    >{showConPassword ? <IoEyeOutline className="text-xl" /> : <IoEyeOffOutline className="text-xl" />}
                    </button>
                </div>
